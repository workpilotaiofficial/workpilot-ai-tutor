...
export const exploreStudySetType = defineType({ name: 'exploreStudySet', title: 'Explore study set', type: 'document', fields: [
  ...
  defineField({ name: 'featured', type: 'boolean', initialValue: false }), defineField({ name: 'sortOrder', type: 'number', initialValue: 0 }), defineField({ name: 'sections', type: 'array', of: sections, validation: (r) => r.required().min(1) }), defineField({ name: 'seo', type: 'seo' }),
] })
